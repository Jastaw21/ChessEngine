...
TEST(BoardManagerAdvancedRules, CheckCantBeExposed){
    ...
    EXPECT_TRUE(BPMovesOutOfWay.resultBits & MoveResult::ILLEGAL_MOVE);
}
...
TEST(BoardManger, PromotionMovesGen){
    auto move = createMove(WP, "a7a8Q");
    EXPECT_EQ(move.promotedPiece, WQ);
    EXPECT_EQ(move.toUCI(), "a7a8Q");
}

TEST(BoardManger, PromotionUpdatesBoardState){
    auto manager = BoardManager();
    manager.setFullFen("8/P6k/8/8/7r/8/3B4/5K2 w - - 0 1"); // the a pawn can promote
    auto move = createMove(WP, "a7a8Q");

    EXPECT_TRUE(manager.tryMove(move));

    EXPECT_TRUE(manager.getBitboards()->getPiece(56).has_value());
    EXPECT_EQ(manager.getBitboards()->getPiece(56).value(), WQ);
    EXPECT_EQ(manager.getFullFen(), "Q7/7k/8/8/7r/8/3B4/5K2 b KQkq - 0 1");
}

TEST(BoardManger, PromotionCanBeUndone){
    auto manager = BoardManager();
    manager.setFullFen("8/P6k/8/8/7r/8/3B4/5K2 w - - 0 1"); // the a pawn can promote
    auto startingPosOnlyFen = manager.getFullFen();
    auto move = createMove(WP, "a7a8Q");

    EXPECT_TRUE(manager.tryMove(move));

    manager.undoMove();
    EXPECT_EQ(manager.getFullFen(), startingPosOnlyFen);
}

TEST(BoardManager, EnPassantSquareUpdated){
    auto manager = BoardManager();
    manager.setFullFen(Fen::FULL_STARTING_FEN);
...
TEST(BoardManager, EnPassantSquareUndone){
    auto manager = BoardManager();
    ...
    manager.resetGame(Fen::FULL_STARTING_FEN);
